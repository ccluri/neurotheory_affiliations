...
def victor_exceptions_2018(filename='affiliationsNotInDB_2018.csv', exceptions_filename='exceptions_alias.csv'):
    already_parsed = {}
    if os.path.isfile(exceptions_filename):
        with open('exceptions_alias.csv', 'rt') as csvfile:
            spamreader = csv.reader(csvfile, delimiter=':')
...
def victor_2018(filename='affiliationsNotInDB_2018.csv', new_filename='resolvedAff2018.csv'):
    name_exceptions = utils.exceptions_alias()
    with open(filename, 'rt') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        aff_list = []
    ...
    with open(new_filename, 'wt') as csvfile:
        writer = csv.writer(csvfile, delimiter=',', quoting=csv.QUOTE_NONNUMERIC)
        writer.writerow(['affiliation', 'Aff resolved', 'GRID'] +
                        ['City', 'Country', 'Latitude', 'Longitude'])
        for idx, gid in enumerate(gid_list):
            if gid != '':
                writer.writerow([aff_list[idx], g_i[gid], gid, city_list[idx], country_list[idx]] +
                                [str(jj) for jj in gid_latlong_dict[gid]])
            else:
                writer.writerow([aff_list[idx], '', '', '', ''] +
                                ['', ''])
